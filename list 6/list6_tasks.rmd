...
The experiment proves that all of tested statistics actually converge to their asymptotic distributions. In particular, the approximations are very close to the ground-truth values when the number of samples is equal to at least $30$. When the number of samples is set to a higher value than $30$, the quality of approximations doesn't always improve. The reason is that only $10000$ simulations were used to estimate the critical values, thus it impacts the approximation errors.


# Task 2

Taking account that we have access only to a PDF $f$ of a distribution, we can use the elimination method to generate random samples from the distribution. The elimination method involves defining a constant $M$ and a PDF $g$ such that $f(x) \leq Mg(x)$ for all $x$ i.e. $Mg(x)$ is an upper bound of the PDF $f$. One additional constraint is that we should be able to generate samples from a distribution given by the PDF $g$. The next step of the elimination method is to sample $X \sim g$ and $U \sim U[0, 1]$. If $U \leq \frac{f(X)}{Mg(X)}$, we accept $X$ as a sample from the distribution given by the PDF $f$. Otherwise, we repeat our sampling procedure until the equation is not satisfied.

The intuition behind the elimination method is that variables $X, U$ represents a $(x, y)$ point on a plane. The method accepts a point as a sample if it is under the PDF $f$. Otherwise, a new point $(x, y)$ is sampled. In practice, if we are not able to come up with a good upper bound of the PDF $f$ and the support of $f$ is limited by $[a, b]$, we can set $g \sim U[a, b]$ and $M = \max_x f(x)$. In particular, the only role of the PDF $g$ is to increase the acceptance ratio of the elimination method.
...
The first outcome from the experiment is that increasing the number of samples impacts  power of a test significantly. Particularly, when the number of samples is set to $10$, the best test has a power below $20\%$, while when the number of samples is set to $100$, the best test has a power above $70\%$. The other observation is that the $KS$ test works better than other tests when the number of samples is low. In contrast, when the number of samples is set to at least $30$, the $N_1$ test has the best power. For higher number of samples, the difference between the $N_1$ test and other tests is even higher. Lastly, the performance of the $P_4$ and $P_8$ tests is the worst among all the tests.
...
The main observation is that a power of a test is highly dependent on a distribution. In more detail, there are two cases in which the $N_4$ test has the best power and three cases in which the $N_8$ has the best power. Additionally, the results for the $P_8$ test are close to the best performing tests in all cases. The results of the $P_4$ test are not consistent across different distributions. In particular, the performance is satisfying in 2 cases, while it is very poor in the other cases. Lastly, the performance of the $N_1$ and $KS$ tests is poor in all of the tested cases.

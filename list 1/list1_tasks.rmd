...
count_estimators_statistics <- function(estimators_frame, estimated_paramter, excluded_col_names=c()) {
  ...
  for (col_name in colnames(estimators_frame)) {
    ...
    statistics_frame[paste0(col_name, '.ms')] <- mean((c(estimators_frame[[col_name]]) - estimated_paramter) ^ 2)
    statistics_frame[paste0(col_name, '.var')] <- var(c(estimators_frame[[col_name]]))
    statistics_frame[paste0(col_name, '.bias')] <- mean(c(estimators_frame[[col_name]]) - estimated_paramter)
  }

...
The first insight from the experiment is that the KS test works better than other tests when one distribution is moved by some constant $M$. In particular, the results for the KS test are satisfactory in all of the tested cases. Additionally, the results for the Wilcoxon test are quite good, especially if the constant $M$ is high. In contrast, the AB test fails in all of the tested cases. It is expected as it tests the equality of two distributions based on their variances, whereas in our experiment, all of the pairs of distributions have the same variance. As the Lepage test is based on the Wilcoxon and AB tests, comparing it to the Wilcoxon test, its quality drops.

The other outcome from the experiment is that the difference between the KS test and the Wilcoxon test is especially visible for small values of the constant $M$. In contrast, when the constant $M$ is high enough, both tests have their powers close to $1.0$. The last insight from our experiment is that the powers of tests are higher for the normal distribution compared to the Logistic and Cauchy distributions.
...
The experiment shows that the AB test dominates the other tests in nearly all of the tested cases. The difference is especially visible when a difference between standard deviations of a pair of tested distributions is high. This phenomenon is quite expected as the AB test is specialized in the detection of differences in variances. Contrary to the AB test, the Wilcoxon test completely fails to detect a difference between a pair of distributions. As a result, the performance of the Lepage test is worse than the performance of the AB test. The KS test works well in all of the cases, but its performance is worse than the performance of the AB test on average.
...
The results of the experiment suggests that the KS and Lepage tests work better than other tests when there is a difference in both means and variances of two distributions. Particularly, the performance of the KS test is significantly better than the performance of the other tests when a difference between a pair of distributions is small. The only test with a poor performance is the Wilcoxon test. While its performance increases when a difference between a pair distributions is higher, it is noticeably poorer than the performance of other tests. Despite the fact that the performance of the Wilcoxon test is poor, it affects the Lepage test in a positive way. In particular, the performance of the Lepage is better than the performance of the AB test in most of the tested cases.
...
While the quality of the approximations for the Wilcoxon, AB and Lepage tests decreased a bit, it can be a coincidence as the errors were already pretty low when $20$ samples were used. In contrast, the approximation for the critical value of the KS test is better compared to the case when $20$ samples were used. In this case, the relative error was much higher compared to the other tests, thus it can be assumed that increasing the number of samples actually improved our approximation.
